...
class Circuit:
    ...
    def evaluate_variance_min(self, record_data = False):
        """
        Upload and run this Circuit on the FPGA and analyze its
        performance.
        """
        ...
        waveform = self.__read_variance_data()
        fitness = self.__measure_variance_min_fitness(waveform)
    ...
    def __measure_variance_min_fitness(self, waveform):
        ...
        for i in range(len(waveform)):
            ...
            initial1 = waveform[i] #int(data[i].strip().split(b": ", 1)[1])
            # Take the absolute difference of the two points and store to a variable
            variance = abs(initial1 - 200) if i < 250 else abs(initial1 - 600)
        ...
        var_min_fitness = variance_sum / total_samples
        if var_min_fitness == 0:
            self.__fitness = 1
        else:
            self.__fitness = 1 / var_min_fitness
